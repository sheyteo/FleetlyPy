...
class Scenario:
    ...
    def assignVehicles(self):
        ...
        for vehicle in vehicles:
            ...
            for customer in customers:
                if customer["id"] in self.cid_set:
                    tempArr.append(self.costFunction(vehicle, customer))
                    tempArr2.append((vehicle["id"], customer["id"]))

            idMatrix.append(tempArr2)
            matrix.append(tempArr)


        hungarian = Hungarian(matrix)
        retMatrix = hungarian.calculate()
        retMatrix = hungarian.get_results()
        for (idx, idy) in retMatrix:
            x,y =idMatrix[idx][idy]

            aList.append({"id": x,
             "customerId": y})

            self.cid_set.remove(y)
